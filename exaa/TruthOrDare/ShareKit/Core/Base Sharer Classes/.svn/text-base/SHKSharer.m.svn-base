...
+ (id)shareItem:(SHKItem *)i
{
    ...
    appDelegate.navigationController.view.userInteractionEnabled = NO;
...
{
    TruthOrDareAppDelegate *appDelegate = (TruthOrDareAppDelegate *)[[UIApplication sharedApplication] delegate];
    appDelegate.navigationController.view.userInteractionEnabled = YES;
    
	if (!quiet)
		[[SHKActivityIndicator currentIndicator] displayActivity:SHKLocalizedString(@"Connecting.. %@", [[self class] sharerTitle])];
}
...
- (void)sharerFinishedSending:(SHKSharer *)sharer
{
	...
	if (!quiet)
		[[SHKActivityIndicator currentIndicator] displayCompleted:SHKLocalizedString(@"Finished!")];
}
